...
/*	fd->fd_omode" 表示该文件描述符的打开模式。打开模式包括 "O_RDONLY"（只读）、"O_WRONLY"（只写）和 "O_RDWR"（读写）。因此，该条件语句首先检查文件描述符的打开模式是否为只写模式（即是否等于 "O_WRONLY"）。如果是，它返回错误码 "-E_INVAL"，表示该操作无效。

这里之所以使用 "fd->fd_omode" 而不是 "fd->fd_mode"，是因为 "fd_mode" 是文件的访问权限，而不是打开模式。打开模式是在打开文件时指定的，它决定了文件可以进行的操作类型，而访问权限则决定了谁可以对文件进行这些操作。
*/
	// Step 3: Read from 'dev' into 'buf' at the seek position (offset in 'fd').
	/* Exercise 5.10: Your code here. (3/4) */
	 r = dev->dev_read(fd, buf, n, fd->fd_offset);
	 /*
这段代码调用了一个函数指针，这个函数指针指向设备驱动程序中的 "dev_read" 函数。该函数的参数包括 "fd"（文件描述符结构体指针）、"buf"（缓冲区指针）、"n"（要读取的字节数）和 "fd->fd_offset"（读取偏移量）。
*/
