...
//	其分配的时间片（count为0）
	if (yield || count == 0 || e == NULL || e->env_status != ENV_RUNNABLE) {
        if (e != NULL && e->env_status == ENV_RUNNABLE) {
            // Move the current env to the tail of env_sched_list
            TAILQ_REMOVE(&env_sched_list, e, env_sched_link);
            TAILQ_INSERT_TAIL(&env_sched_list, e, env_sched_link);
        }

        // Select the next runnable environment
        e = TAILQ_FIRST(&env_sched_list);
        if (e == NULL)
            panic("schedule: no runnable environments!");

        // Update the remaining time slices
        count = e->env_pri;
    }

    // Decrease the remaining time slices by 1
    count--;

    // Schedule the selected environment
    env_run(e);
