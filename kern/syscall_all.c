...
int sys_set_tlb_mod_entry(u_int envid, u_int func) {
	...
	try(envid2env(envid,&env,1) == -E_BAD_ENV);
...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	int r = envid2env(srcid, &srcenv, 1); 
	if (r < 0) {
		return -E_BAD_ENV;
	}
	...
	if (envid2env(dstid, &dstenv, 1) == -E_BAD_ENV) {
		return -E_BAD_ENV;
	}
...
int sys_exofork(void) {
	...
	try(env_alloc(&e,curenv->env_id) == -E_NO_FREE_ENV);	       



	/* Step 2: Copy the current Trapframe below 'KSTACKTOP' to the new env's 'env_tf'. */
	/* Exercise 4.9: Your code here. (2/4) */
	e->env_tf = *((struct Trapframe *) KSTACKTOP-1);//运行现场 要复制一份当前进程的运行现场（进程上下文）Trapframe 到子进程的进程控块中

	/* Step 3: Set the new env's 'env_tf.regs[2]' to 0 to indicate the return value in child. */
	/* Exercise 4.9: Your code here. (3/4) */
	e->env_tf.regs[2] = 0;//返回值有关 这个系统调用本身是需要一个返回值的，我们希望系统调用在内核态返回的envid 只传递给父进程，
			      //对于子进程，则需要将其现场中的 v0 寄存器修改为 0

	/* Step 4: Set up the new env's 'env_status' and 'env_pri'.  */
	/* Exercise 4.9: Your code here. (4/4) */
	e->env_status = ENV_NOT_RUNNABLE;//所以我们需要将它的状态设为 ENV_NOT_RUNNABLE 且避免它被加入调度队列。
	e->env_pri = curenv->env_pri;//这些字段的初始值应该是继承自父进程还是使用新的值
...
int sys_set_env_status(u_int envid, u_int status) {
	...
	//这个函数的第三个参数为0，表示不允许将进程控制块的权限改变为只读。如果将该参数设置为1，则允许改变进程控制块的权限为只读

	/* Step 3: Update 'env_sched_list' if the 'env_status' of 'env' is being changed. */
	/* Exercise 4.14: Your code here. (3/3) */ 
	u_int ori_status = env->env_status;
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	/* Exercise 4.8: Your code here. (5/8) */ //对的
	...
	/* Exercise 4.8: Your code here. (6/8) */ //对的
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	/*
	   ...
	   if((pa >= 0x10000000 && pa <0x10000020) || (pa >= 0x13000000 && pa < 0x13004200) ||(pa >= 0x15000000 && pa < 0x15000200)){
	   if ( pa >= 0x10000000 && pa < 0x10000020){
	   if(( pa + len) < pa || (pa+len > 0x10000020) return -E_INVAL;
	   }
	   if ( pa >= 0x13000000 && pa < 0x1300420){
	   if(( pa + len) < pa || (pa+len > 0x13004200) return -E_INVAL;		
	   }
	   if ( pa >= 0x15000000 && pa < 0x1500200){ 
	   if(( pa + len) < pa || (pa+len > 0x15000200) return -E_INVAL;}
	   else{
	   return -E_INVAL;
	   memcpy((void *)pa, (void *)va, len);
	   return 0;
	   }
	   */
	int err = 1;
	if (is_illegal_va_range(va, len)) err =0;
	if (pa >= 0x10000000 && pa + len-1 < 0x10000020) err =0; 
	if   ( pa >= 0x13000000 && pa + len-1 < 0x13004200) err = 0;
	if( pa >= 0x15000000 && pa + len-1 < 0x15000200) err = 0; 
