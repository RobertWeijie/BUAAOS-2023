...
void mips_detect_memory() {
    ...
    npage = memsize >> PGSHIFT;
	  printk("Memory size: %dK available, ", (int)(memsize / 1024));
	  printk("number of pages: %d",(int)(npage));
} 
...
void page_init(void) {
	...
	//声明一个变量 `used`，表示空闲内存以下的页面数，初始值为 `(freemem-ULIM)/BY2PG`
//声明一个计数器变量 `i`，并初始化为 0
//遍历前 `used` 个页面，将它们的 `pp_ref` 字段设置为 1，表示这些页面已被占用
	...
	} //pp_link 表示下一个空闲页,如果pp_link=0,则表示这个页面被分配了,否则,这个页面未被分配,是空闲页面
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	*ppte = (Pte*)KADDR(PTE_ADDR(*pgdir_entryp))+PTX(va);;//由于页表项中存储的是物理地址，因此需要先将页表项中的物理地址提取出来，并将其转换为虚拟地
